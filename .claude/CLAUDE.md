...
- Sole dependency is [gomatic/go-error](https://github.com/gomatic/go-error): `ErrInvalidPair` is an `errs.Const`, matched with `errors.Is`; never `errors.New`, `fmt.Errorf` only to `%w`-wrap.
